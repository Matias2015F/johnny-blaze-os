  ...
  return (
    <div className="min-h-screen bg-zinc-100 text-left animate-in slide-in-from-right duration-300 pb-32">
          ...
          <div className="text-left">
            ...
            <div className="flex gap-4 mt-4">
              <div className="bg-white/10 px-4 py-2 rounded-2xl">
                <p className="text-[8px] font-black uppercase text-zinc-400">Cliente</p>
                <p className="text-xs font-black">{c?.nombre || "---"}</p>
              </div>
              <div className="bg-white/10 px-4 py-2 rounded-2xl">
                <p className="text-[8px] font-black uppercase text-zinc-400">Km Actual</p>
      ...
      <div className="p-6 space-y-4">
        ...
        {alertasMoto.length > 0 && (
          <div className="space-y-2">
            <p className="text-[10px] font-black text-yellow-600 uppercase tracking-widest flex items-center gap-2">
              <Bell size={12} /> Pr�ximo control sugerido
            </p>
            {alertasMoto.map(r => (
              <div key={r.id} className={`rounded-[2rem] p-5 border space-y-3 ${r.estadoAlerta === "service_vencido" ? "bg-red-50 border-red-200" : "bg-yellow-50 border-yellow-200"}`}>
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-lg inline-block mt-1 ${r.estadoAlerta === "service_vencido" ? "bg-red-500 text-white" : "bg-yellow-400 text-black"}`}>
                      {r.estadoAlerta === "service_vencido" ? "Vencido" : "Pr�ximo"}
                    </span>
                    {r.testMode && (
                ...
                {r.kmObjetivo != null && (
                  <div className="bg-white/60 rounded-xl p-3 space-y-1">
                    ...
                    <div className="flex justify-between text-[9px] font-black text-zinc-700 uppercase border-t border-zinc-200 pt-1 mt-1">
                      <span>Objetivo</span><span>{(r.kmObjetivo || 0).toLocaleString("es-AR")} km</span>
                    </div>
                  </div>
                )}
                {r.fechaObjetivo && !r.kmObjetivo && (
                  <p className="text-[9px] font-bold text-zinc-500">Fecha objetivo: {r.fechaObjetivo}</p>
                )}
                <div className="flex gap-2">
                  <button
                    onClick={() => LS.updateDoc("recordatorios", r.id, { estado: "hecho" })}
                    className="flex-1 py-2.5 rounded-xl bg-zinc-200 text-zinc-700 text-[9px] font-black uppercase active:scale-95"
                  >
                    Marcar hecho
                  </button>
                  {r.testMode && (
        ...
        <h3 className="text-xs font-black uppercase text-zinc-400 tracking-widest px-2">Historial de Servicios</h3>
        {history.length === 0 && (
          <p className="text-center py-10 text-zinc-400 font-bold uppercase text-[10px]">Sin servicios registrados</p>
        )}
        {history.map((order) => {
          ...
          return (
            <div key={order.id} className="bg-white rounded-[2.5rem] border border-zinc-200 overflow-hidden shadow-sm">
              <div onClick={() => setExpandedId(isExpanded ? null : order.id)} className="p-6 flex justify-between items-center cursor-pointer active:bg-zinc-50">
                <div className="text-left">
                  <p className="text-xs font-black text-zinc-400 uppercase mb-1 tracking-widest">{order.fechaIngreso}</p>
                  <p className="text-xl font-black text-black leading-none">{formatMoney(totalOrden)}</p>
                  <p className="text-[10px] font-bold text-zinc-500 mt-1 uppercase">Kilometraje: {order.km} km</p>
                </div>
              ...
              {isExpanded && (
                <div className="px-6 pb-6 pt-2 border-t border-zinc-50 space-y-4 animate-in fade-in slide-in-from-top-2">
                  <div className="space-y-3">
                    <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Trabajos realizados y repuestos</p>
                    <div className="space-y-1">
                      {order.tareas?.map((t, i) => (
                        <div key={i} className="text-xs font-bold text-zinc-700 uppercase flex items-center gap-2">
                          <div className="w-1.5 h-1.5 rounded-full bg-orange-500" /> {t.nombre}
                        </div>
                      ))}
                      {order.repuestos?.map((r, i) => (
                        <div key={i} className="text-xs font-bold text-orange-700 uppercase flex items-center justify-between gap-2">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="w-1.5 h-1.5 rounded-full bg-orange-500" /> <span className="truncate">{r.nombre} ({r.cantidad || 1}x)</span>
                          </div>
                          <span className="text-[10px] text-orange-500 flex-shrink-0">{formatMoney((r.monto || 0) * (r.cantidad || 1))}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                  {(order.repuestos?.length > 0 || order.insumos?.length > 0) && (
                    <div className="space-y-3">
                      {order.repuestos?.length > 0 && (
                        <div className="space-y-2">
                          <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Historial de repuestos</p>
                          {order.repuestos.map((r, i) => (
                            <div key={i} className="bg-orange-50 p-3 rounded-2xl border border-orange-100 text-xs">
                              <div className="flex justify-between gap-3">
                                <span className="font-black text-orange-900 uppercase">{r.nombre}</span>
                                <span className="font-black text-orange-600">{formatMoney(r.monto || 0)}</span>
                              </div>
                      ...
                      {order.insumos?.length > 0 && (
                        <div className="space-y-2">
                          <p className="text-[10px] font-black uppercase text-zinc-400 tracking-widest">Historial de gastos e insumos</p>
                  ...
                  {order.diagnostico && (
                    <div className="bg-zinc-50 p-4 rounded-2xl border border-zinc-100">
                      <p className="text-[8px] font-black uppercase text-zinc-400 mb-1">Diagn�stico</p>
                      <p className="text-xs font-bold italic text-zinc-600">"{order.diagnostico}"</p>
                    </div>
                  )}
                  {order.observacionesProxima && (
                    <div className="bg-orange-50 p-4 rounded-2xl border border-orange-100">
                      <p className="text-[8px] font-black uppercase text-orange-400 mb-1">Notas para la pr�xima visita</p>
