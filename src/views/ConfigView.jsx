    ...
    <div className="p-6 text-left animate-in slide-in-from-right duration-300 pb-28">
      ...
      <div className="space-y-4 bg-white p-8 rounded-[2.5rem] shadow-xl">
        <div className="space-y-1">
          ...
          <input value={cfg.nombreTaller} onChange={(e) => setCfg({ ...cfg, nombreTaller: e.target.value })} className="w-full border-2 border-slate-100 rounded-2xl p-4 font-black outline-none focus:border-blue-500" />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Responsable</label>
          <input value={cfg.mecanicoResponsable} onChange={(e) => setCfg({ ...cfg, mecanicoResponsable: e.target.value })} className="w-full border-2 border-slate-100 rounded-2xl p-4 font-black outline-none focus:border-blue-500" />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">DNI</label>
          <input value={cfg.dniMecanico} onChange={(e) => setCfg({ ...cfg, dniMecanico: e.target.value })} className="w-full border-2 border-slate-100 rounded-2xl p-4 font-black outline-none focus:border-blue-500" />
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Teléfono</label>
          <input value={cfg.telefonoTaller} onChange={(e) => setCfg({ ...cfg, telefonoTaller: e.target.value })} className="w-full border-2 border-slate-100 rounded-2xl p-4 font-black outline-none focus:border-blue-500" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Hora Costo</label>
            <input type="number" value={cfg.valorHoraInterno} onChange={(e) => setCfg({ ...cfg, valorHoraInterno: Number(e.target.value) })} className="w-full border-2 border-slate-100 rounded-2xl p-4 font-black outline-none focus:border-blue-500" />
          </div>
          <div className="space-y-1">
            <label className="text-[10px] font-black text-slate-400 uppercase ml-2 tracking-widest">Hora Cliente</label>
            <input type="number" value={cfg.valorHoraCliente} onChange={(e) => setCfg({ ...cfg, valorHoraCliente: Number(e.target.value) })} className="w-full border-2 border-slate-100 rounded-2xl p-4 font-black outline-none focus:border-blue-500" />
          </div>
        </div>
        <button onClick={guardar} className="w-full bg-blue-600 text-white py-5 rounded-3xl font-black uppercase shadow-xl active:scale-95 transition-all mt-4">
          Guardar Cambios
        </button>
      </div>
    </div>
