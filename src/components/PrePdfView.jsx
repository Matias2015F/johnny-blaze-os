    ...
    <div className="p-6 text-left animate-in fade-in pb-32">
      ...
      <div className="bg-white p-8 rounded-[2.5rem] space-y-6 shadow-2xl">
        <div className="bg-orange-50 border-2 border-orange-200 p-4 rounded-3xl flex items-center gap-3">
          ...
          <p className="text-[10px] font-black text-orange-700 uppercase leading-tight">Al generar este PDF, la orden se BLOQUEARÁ para evitar cambios de último momento.</p>
        </div>
        <div>
          ...
          <div className="flex gap-2 mt-2 overflow-x-auto pb-2">
            {PLANTILLAS_GARANTIA.map((p) => (
              <button key={p.id} onClick={() => setGarantia(p.texto)} className="bg-slate-100 px-4 py-2 rounded-xl text-[10px] font-black uppercase whitespace-nowrap active:bg-orange-500 active:text-white transition-colors">
                {p.nombre}
              </button>
            ))}
          </div>
        ...
        <div>
          ...
          <textarea value={garantia} onChange={(e) => setGarantia(e.target.value)} rows="5" className="w-full border-2 border-slate-100 rounded-2xl p-4 font-bold text-sm text-slate-700 outline-none focus:border-orange-500 mt-2" />
        </div>
        <button onClick={irAlPdf} className="w-full bg-orange-600 text-white py-6 rounded-3xl font-black uppercase shadow-xl flex items-center justify-center gap-3 active:scale-95 transition-all">
