  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4 font-sans text-left text-slate-900 animate-in fade-in">
      <div className="mx-auto max-w-[960px] bg-white print:max-w-none">
        ...
        <div className="border-b-2 border-slate-900 px-12 py-8 print:py-10">
          <div className="flex items-start justify-between gap-8">
            {/* DATOS TALLER - IZQUIERDA */}
            <div className="flex-1">
              <p className="text-[9px] font-black uppercase tracking-[0.2em] text-blue-600">
                Constancia de servicio técnico
              </p>
              <h1 className="mt-3 text-2xl font-black uppercase tracking-tight leading-tight max-w-xs">
                {config.nombreTaller}
              </h1>
            ...
            <div className="flex flex-col items-center gap-4">
              ...
              {snapshot.hash && (
                <div className="rounded-lg border-3 border-slate-900 bg-white p-2 print:p-3">
                  <QRCodeCanvas
        ...
        {/* CONTENIDO PRINCIPAL */}
        <div className="px-12 py-6 print:py-8 space-y-6">
          ...
          {/* TRABAJOS REALIZADOS */}
          <div>
            <h3 className="text-[10px] font-black uppercase text-slate-700 tracking-wide mb-2">Trabajos realizados</h3>
            <div className="border border-slate-300">
              <div className="bg-slate-900 px-4 py-2">
                <p className="text-[9px] font-black text-white uppercase tracking-wide">Descripción • Monto</p>
              </div>
              {tareas.length > 0 ? (
                tareas.map((t, i) => (
                  <div key={i} className={`flex justify-between px-4 py-3 text-sm ${i < tareas.length - 1 ? 'border-b border-slate-200' : ''}`}>
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bold">Mano de obra</p>
                    </div>
          ...
          {repuestos.length > 0 && (
            <div>
              <h3 className="text-[10px] font-black uppercase text-slate-700 tracking-wide mb-2">Repuestos utilizados</h3>
              <div className="border border-slate-300">
                <div className="bg-slate-100 px-4 py-2 border-b border-slate-300">
                  <p className="text-[9px] font-black text-slate-700 uppercase tracking-wide">Descripción • Monto</p>
                </div>
                {repuestos.map((r, i) => (
                  <div key={`rep-${i}`} className={`flex justify-between px-4 py-3 text-sm ${i < repuestos.length - 1 ? 'border-b border-slate-200' : ''}`}>
                    <div>
                      ...
                      <p className="text-[10px] text-slate-500 font-bold">{r.cantidad || 1} × {formatMoney(r.monto || 0)}</p>
                    </div>
          ...
          {pagos.length > 0 && (
            <div>
              <h3 className="text-[10px] font-black uppercase text-slate-700 tracking-wide mb-2">Pagos registrados</h3>
                ...
                <tbody>
                  {pagos.map((p, i) => (
                    <tr key={i} className={i < pagos.length - 1 ? 'border-b border-slate-200' : ''}>
                      ...
                      <td className="px-4 py-2 text-[10px] font-bold text-slate-700 uppercase">{labelMetodo(p.metodo)}</td>
                      <td className="px-4 py-2 text-[10px] text-slate-700">{p.comprobante || "---"}</td>
                      <td className="px-4 py-2 text-[10px] font-black text-slate-900 text-right">{formatMoney(p.monto || 0)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="border border-slate-300 p-4 space-y-2">
              ...
              <div className={`flex justify-between px-3 py-3 mt-3 font-black text-white rounded ${saldo <= 0 ? 'bg-green-600' : 'bg-slate-900'}`}>
          ...
          <div className="bg-yellow-50 border-2 border-yellow-300 p-4 space-y-2">
            <p className="text-[10px] font-black uppercase text-yellow-800 tracking-wide">⚠️ Términos de garantía y validez</p>
