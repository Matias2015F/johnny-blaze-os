...
| T7 | isPlatformAdmin() realiza lectura Firestore por cada operación, amplificando costos y creando vector de DoS económico en escala | remote_auth | Firestore client SDK, Admin Dashboard | Disponibilidad del servicio | medium | possible | mitigated | Custom Claims `{ admin: true }` — cero lecturas Firestore desde `a846c0d`; assertAdmin() en `_firebase-admin.js` sincrónico | |
...
| ~~Migrar admin check a Firebase Custom Claims~~ — **DONE** `a846c0d` | T1, T7 | partial | M |
